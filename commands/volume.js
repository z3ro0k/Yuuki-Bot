...
exports.run = async (client, message, args, queue) => {
    ...
    if (!message.member.voiceChannel) return message.channel.send('Usted no está en un canal de voz! :loud_sound:');
    if (!serverQueue) return message.channel.send('No hay nada reproduciéndose');
    if (!args1[1]) return message.channel.send(`<a:volume:458742308867932160> el volumen actual es: **${serverQueue.volume}**`);
    serverQueue.volume = args1[1];
    if (args1[1] > 10) return message.channel.send(':no_entry_sign: Se ha alcanzado el límite de `10`.');
    serverQueue.connection.dispatcher.setVolumeLogarithmic(args1[1] / 10);

    let vEmbed = new Discord.MessageEmbed()
        .setDescription(`<a:volume:458742308867932160> Volumen configurado a: **${args1[1]}**`)
        .setColor(0x36393e)
